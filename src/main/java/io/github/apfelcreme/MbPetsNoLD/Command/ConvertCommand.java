...
public class ConvertCommand implements SubCommand {
    ...
    public void execute(ChatInput chatInput) {
        if (!chatInput.getSender().hasPermission("MbPets.convert")) {
        ...
        if (MbPets.getInstance().getPluginAnimalProtect() == null) {
            MbPets.getInstance().getLogger().info("Plugin 'AnimalProtect' was not found");
            //return;
        }
        MbPets.getInstance().getConvertRightclickListener().addConvert(chatInput.getSender());
