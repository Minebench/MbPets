...
public class DeleteCommand implements SubCommand {
    ...
    public void execute(final ChatInput chatInput) {
        ...
        int number = (chatInput.getNumber() == null ? PetManager.getInstance()
                .getHighestPetNumber(chatInput.getTargetPlayer().getUniqueId())
                : chatInput.getNumber());
        Pet pet = PetManager.getInstance().loadPet(chatInput.getTargetPlayer().getUniqueId(), number);
        if (pet != null) {
            pet.delete();
            MbPets.sendMessage(chatInput.getSender(), MbPetsConfig.getTextNode("info.petDeleted"));
        }
        if (PetManager.getInstance().getPets().get(chatInput.getSender().getUniqueId()) != null) {
            PetManager.getInstance().getPets().get(chatInput.getSender().getUniqueId()).uncall();
        }
    }
}
