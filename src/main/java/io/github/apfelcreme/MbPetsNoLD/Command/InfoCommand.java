...
public class InfoCommand implements SubCommand {
    ...
    public void execute(final ChatInput chatInput) {
        ...
        if (pet != null) {
            MbPets.sendMessage(chatInput.getSender(), pet.getPetDescription().getDescription());
        } else {
            MbPets.sendMessage(chatInput.getSender(), MbPetsConfig.getTextNode("error.noPetToCall")
                    .replace("{0}", "Du"));
        }
    }
}
